<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&pause=1000&color=F7F7F7&background=1A1B27&center=true&vCenter=true&width=800&lines=Hola%2C+soy+Juan+Flores!;Ingeniero+de+Sistemas+%F0%9F%92%BB;L%C3%ADder+T%C3%A9cnico+y+Desarrollador+%F0%9F%92BB;Creador+de+Contenido+Deportivo+%F0%9F%8F%86;Especialista+en+An%C3%A1lisis+de+Datos+%F0%9F%93%8A" alt="Typing SVG" />
</p>

<h1 align="center">👋 Hola, soy Juan Flores</h1>
<h3 align="center">💻 Ingeniero de Sistemas · 🚀 Líder de Equipo de Desarrollo · ⚽ Creador de Contenido Deportivo</h3>

---

## 🔗 Encuéntrame en la Red
* **LinkedIn**: [linkedin.com/in/juanflores1901](https://www.linkedin.com/in/juanflores1901)
* **GitHub**: [github.com/JJWALLY1901](https://github.com/JJWALLY1901)
* <kbd>Email</kbd>: `juan.flores@example.com` *(Considera añadir tu correo aquí)*

---

## 🚀 Experiencia Profesional

### **Fundación CALMA**
#### **Jefe de Equipo de Desarrollo** | *ago 2025 - Presente* (1 mes)
* Liderazgo de equipo multidisciplinario utilizando **metodología Scrum**.
* Coordinación y gestión de **sprints**, incluyendo asignación de tareas y seguimiento de progreso.
* Comunicación constante con Scrum Master, Líder Técnico y QA para asegurar la fluidez del proyecto.
* Enfoque en la **resolución de bloqueos** y promoción de un ambiente colaborativo y productivo.

#### **Desarrollador Frontend** | *may 2025 - ago 2025* (4 meses)
* Desarrollo de interfaces web modernas y responsivas con **React.js**, HTML5, CSS3 y JavaScript.
* Integración robusta de **APIs RESTful** para sistemas clínicos y de gestión interna.
* Optimización continua de la **Experiencia de Usuario (UX/UI)** y aseguramiento del cumplimiento de estándares de accesibilidad (**WCAG**).

### **Proyectos de Contenido Digital**
* **El Hincha Blanquiazul** (Fundador | 2020-Presente): Principal voz de análisis y noticias, alcanzando **120M+ de impacto anual**.
* **Gallo Freestyle** (Fundador | 2024-Presente): Creación y viralización de contenido de alto impacto para el nicho de *freestyle* en **LATAM**.
* **Dale Fútbol Perú** (Fundador | 2024-Presente): Plataforma especializada en la cobertura de fútbol, **acreditado oficialmente** en la Liga 1 2025.

---

## 🛠️ Habilidades Técnicas
<p align="center">
  <img src="https://skillicons.dev/icons?i=python,js,html,css,react,git,github,figma,mysql,bash,googleanalytics&theme=dark" />
</p>

* **Desarrollo Frontend**: React.js, JavaScript (ES6+), HTML5, CSS3.
* **Backend/Scripting**: Python, **Bash Shell** para automatización de procesos.
* **Bases de Datos**: MySQL, modelado de datos.
* **Metodologías Ágiles**: Liderazgo, Scrum, Gestión de equipos remotos.
* **Análisis & Métricas**: Google Analytics (incl. E-commerce), Visualización de datos, Estrategias de Engagement.

---

## 🎓 Formación Académica
* **Ingeniería de Sistemas** - Universidad Nacional Federico Villarreal (2021-2025)
* **Programa Internacional de Periodismo Deportivo** - USIL (2021)
* **Inglés Básico** - Centro Universitario de Idiomas UNFV (2025)

---

## 📜 Certificaciones & Conocimientos
* Backend con Python
* Programación en Bash Shell
* Google Analytics para Ecommerce
* Economía Digital: Bitcoin y Criptomonedas
* Economía del Comportamiento

---

## 📊 Estadísticas de GitHub
<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=JJWALLY1901&show_icons=true&theme=radical&hide_rank=false" height="180px"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=JJWALLY1901&layout=compact&theme=radical" height="180px"/>
</div>

---

## 💡 Intereses y Pasiones
* 🧠 **Tech for Good**: Aplicación de la tecnología a la salud mental.
* ⚽ **Data Sports**: Análisis de datos deportivos avanzado (Sports Analytics).
* 📈 **Web Analytics**: Visualización y entendimiento de métricas de redes sociales y web.
* 🤖 **Automatización**: Creación de *scripts* y procesos automatizados para eficiencia.

---

> *"Donde la tecnología encuentra al deporte, nacen nuevas formas de innovar y conectar con audiencias."* - Juan Flores
